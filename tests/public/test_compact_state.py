...
from spectra.cnf import CompactCNFRepairState as CachedCNFRepairState
...
def test_xor_zero_is_not_an_empty_support():
    # 1 xor 2 xor 3 == 0 despite all three literals being true.
    p = CNF(3, ((1, 2, 3),))
    state = CachedCNFRepairState(p, (True, True, True))
    assert state._xors == [0]
    assert state._counts == [3]
    assert state.valid
    state.flip_patch((0, 1))
    assert state.make_break(2) == (0, 1)
    state.flip(2)
