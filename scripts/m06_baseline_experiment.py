#!/usr/bin/env python3
"""M06: preregistered controlled trained baseline experiment on 9x9 Sudoku."""
...
DIAGNOSTIC_RUN_BUDGET_SECONDS = 120.0
...
def count_params(model: torch.nn.Module) -> int:
    return sum(int(p.numel()) for p in trainable_parameters(model))
...
def make_model(
    ...
    *,
    ...
    if kind == "single_pass":
        ...
        # M06 is a direct answer baseline, not a confidence-routing experiment.
        # Freeze the unused confidence head so it does not inflate trainable size.
...
def _heads(block: torch.nn.Module) -> int:
    attn = block.attn
    if hasattr(attn, "num_heads"):
        return int(attn.num_heads)
    if hasattr(attn, "heads"):
        return int(attn.heads)
    raise ExperimentStop(f"cannot read attention head count from {type(attn).__name__}")


def architecture_record(kind: str, model: torch.nn.Module) -> dict[str, Any]:
    if kind in {"fp_recursive", "ternary_recursive"}:
        assert isinstance(model, TRM)
        block_apps = model.N_sup * model.T * (model.n + 1) * len(model.blocks)
        return {
            "kind": kind,
            "class": "TRM",
            "dim": int(model.dim),
            "n_layers": len(model.blocks),
            "heads": _heads(model.blocks[0]),
            "n": int(model.n),
            "T": int(model.T),
            "N_sup": int(model.N_sup),
            "ternary": bool(model.ternary),
            "act8": bool(model.act8),
            "trainable_params": count_params(model),
            "forward_block_applications_per_example": int(block_apps),
        }
    ...
    return {
        ...
        "trainable_params": count_params(model),
        "forward_block_applications_per_example": len(model.blocks),
    }


def dataset_tensors(ds: GridDataset) -> tuple[torch.Tensor, torch.Tensor]:
...
def training_loss(
    ...
    *,
    ...
    return deep_supervision_loss(
        steps, y, lambda_h=0.5, lambda_improve=0.1, margin=0.01
    )
...
def evaluate(
    ...
    *,
    ...
    model.eval()
    x_all, y_all = dataset_tensors(ds)
    pred_chunks: list[torch.Tensor] = []
    with torch.inference_mode():
        for start in range(0, len(ds), BATCH_SIZE):
            xb = x_all[start : start + BATCH_SIZE]
            pred_chunks.append(predict(model, kind, xb, ds.height, ds.width).cpu())
    pred = torch.cat(pred_chunks, dim=0) if pred_chunks else torch.empty_like(y_all)
...
def timing_probe(kind: str, train_ds: GridDataset) -> dict[str, Any]:
    model = make_model(kind, 9090)
    params = trainable_parameters(model)
    opt = torch.optim.AdamW(params, lr=LR, weight_decay=WEIGHT_DECAY)
    x, y = dataset_tensors(train_ds)
    rng = np.random.default_rng(77123)
    q = QuantWarmup(1) if kind == "ternary_recursive" else None
    elapsed = 0.0
    completed = 0
    finite = True
    model.train()
    for step in range(1, TIMING_STEPS + 1):
        ...
        loss = training_loss(model, kind, x[idx], y[idx])
    ...
    return {
        "kind": kind,
        "requested_steps": TIMING_STEPS,
        "completed_steps": completed,
        "elapsed_seconds": elapsed,
        "seconds_per_step": elapsed / max(1, completed),
        "finite": finite and completed == TIMING_STEPS,
        "trainable_params": count_params(model),
    }
...
def derive_budget(timing: list[dict[str, Any]]) -> dict[str, Any]:
    if not timing or not all(bool(r["finite"]) for r in timing):
        return {"run_main": False, "reason": "nonfinite_or_incomplete_timing_probe"}
    s_max = max(float(r["seconds_per_step"]) for r in timing)
    if 6 * TARGET_STEPS * s_max <= MAIN_TRAIN_BUDGET_SECONDS:
        seeds, steps = MODEL_SEEDS, TARGET_STEPS
    elif 3 * TARGET_STEPS * s_max <= MAIN_TRAIN_BUDGET_SECONDS:
        seeds, steps = MODEL_SEEDS[:1], TARGET_STEPS
    else:
        seeds = MODEL_SEEDS[:1]
        steps = min(
            TARGET_STEPS,
            int((MAIN_TRAIN_BUDGET_SECONDS / (3 * s_max)) // 10) * 10,
        )
    return {
        "run_main": steps >= 40,
        ...
        "selected_steps_per_model": int(steps),
        "selected_seeds": list(seeds) if steps >= 40 else [],
        "repeated_seed": len(seeds) > 1 if steps >= 40 else False,
        "estimated_training_seconds": (
            3 * len(seeds) * steps * s_max if steps >= 40 else None
        ),
    }


def snapshot_steps(steps: int) -> set[int]:
    return {
        1,
        max(1, int(round(steps * 0.25))),
        max(1, int(round(steps * 0.50))),
        max(1, int(round(steps * 0.75))),
        steps,
    }
    ...
    torch.save(
        {
            ...
            "quant_strength": (
                quant_strength_state(model) if kind == "ternary_recursive" else {}
            ),
            "git_sha": git_sha(),
        },
        path,
    ...
    if kind == "ternary_recursive":
        ...
        if not restored or any(abs(float(v) - 1.0) > 1e-8 for v in restored.values()):
            raise ExperimentStop(
                "ternary evaluation rejected because restored quantization strength "
                f"is not exactly 1.0: {restored}"
            )
    model.eval()
...
def train_main_model(
    ...
    arch = architecture_record(kind, model)
    params = trainable_parameters(model)
    opt = torch.optim.AdamW(params, lr=LR, weight_decay=WEIGHT_DECAY)
    x, y = dataset_tensors(train_ds)
    rng = np.random.default_rng(int(seed) + 50000)
    qsteps = max(1, steps // 4)
    q = QuantWarmup(qsteps) if kind == "ternary_recursive" else None
    snapshots = snapshot_steps(steps)
    ...
    with curve_path.open("w", encoding="utf-8") as curve:
        ...
        for step in range(1, steps + 1):
            ...
            if not math.isfinite(gv):
                raise ExperimentStop(f"{kind}/seed{seed}: non-finite gradient at step {step}")
            if budget_tracker["used"] > MAIN_TRAIN_BUDGET_SECONDS:
                raise ExperimentStop(
                    f"main training budget exceeded at {kind}/seed{seed}/step{step}"
                )
    ...
    qstate = quant_strength_state(model) if kind == "ternary_recursive" else {}
    if kind == "ternary_recursive":
    ...
    return {
        ...
        "examples_sampled": int(steps * BATCH_SIZE),
        "architecture": arch,
        "forward_block_applications_sampled": int(
            steps * BATCH_SIZE * arch["forward_block_applications_per_example"]
        ),
        "train_seconds": train_seconds,
        ...
        "learning_failure": bool(learning_failure),
        "checkpoint": str(ckpt),
        "quant_warmup_steps": qsteps if q is not None else None,
...
def gradient_inspection(kind: str, train_ds: GridDataset, seed: int) -> dict[str, Any]:
    ...
    x, y = dataset_tensors(train_ds)
    loss = training_loss(model, kind, x[:BATCH_SIZE], y[:BATCH_SIZE])
    loss.backward()
...
def diagnostic_overfit(
    ...
    *,
    ...
    params = trainable_parameters(model)
    opt = torch.optim.AdamW(params, lr=LR, weight_decay=WEIGHT_DECAY)
    q = QuantWarmup(max(1, max_steps // 4)) if kind == "ternary_recursive" else None
    losses: list[float] = []
    started = time.perf_counter()
    for step in range(1, max_steps + 1):
        if time.perf_counter() - started > DIAGNOSTIC_RUN_BUDGET_SECONDS:
            break
        if q is not None:
            q.apply(model, step)
        loss = training_loss(model, kind, x, y, height=height, width=width)
        opt.zero_grad(set_to_none=True)
        loss.backward()
    ...
    model.eval()
    with torch.inference_mode():
        pred = predict(model, kind, x, height, width)
    blank = x == 0
    blank_acc = (
        float((pred[blank] == y[blank]).float().mean().item())
        if int(blank.sum().item()) > 0
        else float("nan")
    )
    exact = float((pred == y).all(dim=1).float().mean().item())
    return {
        "minimal_4x4": bool(minimal),
...
def failure_diagnostics(kind: str, seed: int, train_ds: GridDataset) -> dict[str, Any]:
    x9, y9 = dataset_tensors(train_ds)
    x9, y9 = x9[:8], y9[:8]
    ...
    return {
        ...
        "overfit_8_examples_9x9": diagnostic_overfit(
            kind, x9, y9, height=9, width=9, seed=seed, minimal=False
        ),
        "minimal_pipeline_debug_4x4": diagnostic_overfit(
            kind, x4, y4, height=h4, width=w4, seed=seed, minimal=True
        ),
    }
...
def aggregate_runs(runs: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for kind in KINDS:
        ...
        if not rows:
            result[kind] = {"n": 0}
            continue
        agg: dict[str, Any] = {"n": len(rows), "seeds": [r["seed"] for r in rows]}
        for metric in sorted(rows[0]["test_metrics"]):
            values = [float(r["test_metrics"][metric]) for r in rows]
            agg[metric] = {
                "mean": mean(values),
...
def _fmt_metric(row: dict[str, Any], metric: str) -> str:
    metric_row = row[metric]
    if row["n"] > 1:
        return f"{metric_row['mean']:.4f} ± {metric_row['std_population']:.4f}"
    return f"{metric_row['mean']:.4f} (single seed)"
...
def summary_markdown(summary: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        "Primary endpoint: strict 9×9 Sudoku semantic solve rate. Zero is an admissible result.",
        "",
        "| model | seeds | semantic solve | exact match | blank-cell acc | cell acc |",
        "|---|---:|---:|---:|---:|---:|",
    ]
    for kind in KINDS:
        row = summary.get("aggregate", {}).get(kind, {})
        if not row or not row.get("n"):
            lines.append(f"| {kind} | 0 | n/a | n/a | n/a | n/a |")
        else:
            lines.append(
                f"| {kind} | {row['n']} | {_fmt_metric(row, 'semantic_validity')} | "
                f"{_fmt_metric(row, 'exact_reference_match')} | "
                f"{_fmt_metric(row, 'blank_cell_accuracy')} | "
                f"{_fmt_metric(row, 'cell_accuracy')} |"
            )
    lines += [
        "",
        "Training time, parameter counts, quantization state, learning curves, raw predictions, and failures are retained beside this summary.",
        "",
    ]
    return "\n".join(lines)


def parameter_audit(timing: list[dict[str, Any]]) -> dict[str, Any]:
    counts = {r["kind"]: int(r["trainable_params"]) for r in timing}
    fp, ternary = counts["fp_recursive"], counts["ternary_recursive"]
    relative_gap = abs(fp - ternary) / max(1, fp)
    if relative_gap > 0.01:
        raise ExperimentStop(
            f"recursive models differ by more than 1% trainable parameters: {counts}"
        )
    if counts["single_pass"] <= max(fp, ternary):
        raise ExperimentStop(f"single-pass baseline is not larger: {counts}")
    return {
        "trainable_parameter_counts": counts,
        "recursive_relative_parameter_gap": relative_gap,
        "recursive_match_tolerance": 0.01,
...
def main() -> int:
    ...
    parser.add_argument("--out", default=".m06")
    ...
    if any(audit["cross_split_group_overlap"].values()) or any(
        audit["cross_split_exact_overlap"].values()
    ):
    ...
    param_audit = parameter_audit(timing)
    write_json(out / "parameter_audit.json", param_audit)

    budget = derive_budget(timing)
    budget["parameter_audit"] = param_audit
    ...
    steps = int(budget["selected_steps_per_model"])
    seeds = [int(s) for s in budget["selected_seeds"]]
    tracker = {"used": 0.0}
    ...
    for seed in seeds:
        # Fixed order: floating-point recursive reference first.
        for kind in KINDS:
            try:
                record = train_main_model(
                    kind, seed, steps, train_ds, val_ds, out, tracker
                )
                eval_model, eval_kind, _ = load_checkpoint_for_eval(
                    Path(record["checkpoint"])
                )
                if eval_kind != kind:
                    raise ExperimentStop("checkpoint kind mismatch")
                pred_path = out / "predictions" / f"{kind}_seed{seed}.jsonl"
                record["test_metrics"] = evaluate(
                    eval_model, kind, test_ds, predictions_path=pred_path
                )
                record["evaluation_quant_strength"] = (
                    quant_strength_state(eval_model)
                    if kind == "ternary_recursive"
                    else {}
                )
                record["prediction_file"] = str(pred_path)
                runs.append(record)
                if record["learning_failure"]:
                    diagnostics[f"{kind}_seed{seed}"] = failure_diagnostics(
                        kind, seed, train_ds
                    )
