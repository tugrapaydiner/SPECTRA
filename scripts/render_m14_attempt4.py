#!/usr/bin/env python3
"""Render M14 Attempt-4 result table and Pareto plot from raw evidence."""
...
def main() -> int:
    ap=argparse.ArgumentParser(); ap.add_argument("--experiment",default=".m14a4/experiment"); ap.add_argument("--out",default=".m14a4/rendered")
    ...
    md=out/"result_table.md"; lines=["# M14 Attempt 4 result table","",f"Evidence surface: **{surface}**","",
        "Candidate training weights: **[0.4, 0.3, 0.2, 0.1]** (fixed before Attempt-4 results).","",
        "| config | semantic validity | median complete-solve ms | mean steps | early-exit fraction | role |",
        "|---|---:|---:|---:|---:|---|"]
    for r in table:
    ...
    lines += ["","Complete-solve latency includes the task-specific reference-free semantic stop. Unmatched points are not called iso-budget.",""]
    ...
    for r in table:
        ...
        ax.scatter([x],[y],marker=marker,s=size); ax.annotate(str(r["config_id"]),(x,y),xytext=(5,4),textcoords="offset points",fontsize=8)
    ax.set_xlabel("Median complete-solve latency (ms)"); ax.set_ylabel("Strict semantic solve rate")
    ax.set_title(f"M14 Attempt 4 — {surface} quality / latency"); ax.grid(True,alpha=.25); fig.tight_layout()
    plot=out/"pareto.png"; fig.savefig(plot,dpi=180); plt.close(fig)
    prov={
        "milestone":14,"attempt":4,"surface":surface,"raw_rows":str(raw_path),"raw_rows_sha256":sha256(raw_path),
        "summary":str(summary_path),"summary_sha256":sha256(summary_path),"result_table":str(csv_path),"result_table_sha256":sha256(csv_path),
        "result_table_markdown":str(md),"result_table_markdown_sha256":sha256(md),"pareto_plot":str(plot),"pareto_plot_sha256":sha256(plot),
        "plot_source_of_truth":"raw JSONL rows; pixels are presentation only","primary_candidate":summary.get("selected_candidate"),
        "primary_baseline":summary.get("primary_baseline"),"adaptive_development_attempts_before_confirmation":4,
        "candidate_training_weights":[0.4,0.3,0.2,0.1],"thresholds_changed":False,"semantic_stop_reference_target_used":False,
    }
