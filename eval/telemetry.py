...
def battery_observation() -> dict[str, object]:
    """Return measured battery fields, or explicit unavailability.

    The downstream control policy still requires numeric defaults. Those defaults
    are not observations and must not be used as physical measurement evidence.
    Missing sysfs is normal in CPU containers, and psutil can raise rather than
    return ``None`` there.
    """
