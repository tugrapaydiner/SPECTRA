...
The initial implementation suite at `7ca85b5` passed **1,385 tests**, with **16 slow tests excluded**
...
## Follow-up controls and release guard

The release-intent follow-up adds 11 tests, bringing the local fast suite to
**1,396 passes with 16 slow tests excluded**. Packaging-only changes no longer
request republication of an existing version. Historical measurements above and
in `local_receipt.json` remain the original observations, not rewritten receipts.

Exact final-output preservation is **within the same execution environment**.
Two Intel CI environments each passed all six arms and their own 192 execution
replays. Replaying either CI archive on the local AMD environment fails strict
bitwise output matching, including for the unchanged historical checked runtime:
156 of 192 execution-output comparisons differ per archive. The complete local
diagnostic still finds zero disagreements among the six local runtimes and no
cross-environment decoded-answer or validity differences on these fixtures.
This does not establish bitwise portability across hardware or Python environments.
The strict verifier is not weakened; failed cross-environment runs are retained.

A separate exploratory control expands the same packed weights into dense FP32
and uses PyTorch linear operations with the same recurrent graph and full trace.
Across 32 cases and 448 calls, its ratio of summed case medians to blocked tracing
is 0.308040 (about 3.25x faster still). All 32 cases have different logit bit
patterns; the maximum observed absolute logit difference is 0.00153804. Decoded
answers agree in this small fixture set, but this is not a quality guarantee.
The dim64 additional expanded payload is 197,888 bytes versus 14,712 bytes of owned
blocked payload; neither is peak RSS. This alternative is not promoted into the
